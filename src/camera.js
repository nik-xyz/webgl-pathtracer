class Camera {
    ...
    createCameraMatrix(ratio) {
        ...
        const projX = Math.tan(this.fovy);
        const projY = projX / ratio;

        const matrix = [
            new Vec3(projX, 0, 0).rotateEuler(this.rotation),
            new Vec3(0, projY, 0).rotateEuler(this.rotation),
            new Vec3(0, 0,     1).rotateEuler(this.rotation)
        ];
